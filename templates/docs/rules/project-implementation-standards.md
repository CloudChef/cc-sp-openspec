...
The main thread MUST perform its own comprehensive review and use independent read-only review threads for phase and final implementation review when the runtime supports true parallel execution and the current tool has permission. Start independent review automatically; do not ask the user for extra authorization. Permission means the current runtime/tool policy allows spawning read-only review threads, not that the user must confirm thread startup.
...
- Independent review threads MUST be read-only and findings-only. Findings MUST be concise and structured with priority, evidence location, impact, and suggested fix. They MUST NOT return long narrative summaries or implementation plans.
- Independent review threads SHOULD be started only when true parallel execution is available. If the runtime is sequential-only or fake-parallel, record `parallel_unavailable_or_sequential_runtime` and perform the same scoped review in the main thread instead.
- When the runtime supports persistent reusable review threads, reuse the same read-only review thread for the same `change-id`, repository/worktree, and review role instead of opening a new thread for each implementation task. Typical reusable roles are alignment reviewer, security reviewer, spec/design reviewer, task reviewer, final alignment reviewer, and final security reviewer.
- Reused review threads MUST receive fresh scope, artifacts/diff, checklist, and evidence references for every invocation. They MUST return findings for the current scope only and MUST NOT rely on unstated prior context.
- Review artifacts MUST record the reviewer role, thread identifier/name when available, whether the thread was reused or newly started, reviewed scope, provided artifacts, findings, and restart/fallback reason when reuse is unavailable or unsafe.
- `brainstorm-review.md` MUST record Lightweight Precheck, workflow lane decision, main-process comprehensive or allowed lightweight review, one independent draft brainstorm/context review thread or documented main-thread fallback, cross-validation/finding confirmation, main-thread responses, customer/user confirmation of the lane, and zero unresolved blocking findings before `/sp-spec`.
- `spec-review.md` and `design-review.md` MUST record workflow lane, main-process comprehensive or allowed lightweight review, one independent spec/design review thread or documented main-thread fallback, cross-validation/finding confirmation, main-thread responses, and zero unresolved blocking findings before `/sp-tasks`.
- `tasks-review.md` MUST record workflow lane, main-process comprehensive or allowed lightweight review, one independent task review thread or documented main-thread fallback, cross-validation/finding confirmation, main-thread responses, lane-specific review gates, and zero unresolved blocking findings before `/sp-impl`.
- Full-lane `review.md` MUST record one main-thread full implementation review against all requirements, specs, design, code, tests, and rules.
- Full-lane `review.md` MUST record two independent final implementation review threads when true parallel execution is available, or documented main-thread fallback passes when it is not. Thread 1 checks requirement/spec/design/code alignment. Thread 2 checks security, implementation standards, regressions, logging, data/API/async/config, test quality, and file-size gates.
- Lightweight-lane `review.md` MUST record one scoped main-thread final review plus one reusable read-only reviewer when true parallel execution is available, or documented fallback when it is not. The review checks the Lightweight Precheck, compact contracts, changed code, tests, verification evidence, security-review applicability, and escalation triggers.
- `review.md` MUST record cross-validation/finding confirmation between the main-thread final review and required independent review threads or fallback passes.
