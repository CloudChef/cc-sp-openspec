...
1. Superpower must be installed. Normal phase reviews use Superpower review methods and checklists, but they are performed by the main agent; do not invoke review skills that dispatch subagents during `/sp-brainstorm`, `/sp-spec`, `/sp-tasks`, per-task implementation review, or `/sp-complete`. Only the two final review agents after all `/sp-impl` tasks are complete may use subagent review. If the current agent or runtime does not have those capabilities, use Codex or the current tool's own review capability to complete the same review gates.
...
- `docs/rules/project-implementation-standards.md`: Baseline implementation rules for code paths, requirement scope and fallback control, method parameters/data objects, self-explanatory parameters, exception-object parameter prohibition, map-like parameter prohibition, standalone full verification, real E2E test design and execution, same/equivalent logic reuse, Chinese-by-default generated documentation, file size, existing oversized files that must finish functionality before refactor/split, database runtime, OpenAPI, layering, API IO, and async work.
...
`/sp-code-to-spec` is for existing codebases that are adopting the template for the first time. It generates initial context, `openspec/project.md`, current-state capability specs, current-function business definitions, feature descriptions, feature flow descriptions, feature point / branch matrices, module/capability design baselines, project business rules, language/runtime rules, and standards drafts from current code, tests, configuration, README files, deployment scripts, and existing documents. Current-state specs are project knowledge documents and must be written to `docs/standards/modules/<module>/<module>-<capability>-spec.md`, not `openspec/specs/`; `openspec/` is reserved for project workflow configuration and real change contracts produced by `/sp-spec`. Current-state designs belong under `docs/standards/modules/<module>/<module>-<capability>-design.md`. Every module/capability document path must use real module and feature point/capability names. Module spec and design filenames must include both names and must not use generic names or sequence numbers such as `module-1`, `feature-1`, `capability-001`, `common`, `misc`, `general`, `default`, or `design.md`. Every baseline spec must include the current-function business definition, concrete branches, substantive feature descriptions, and feature flow descriptions. Feature descriptions must state actor, goal, trigger, branch behavior, data/state side effects, observable outcome, evidence, and unknowns. Feature flows must describe the business path from trigger to final observable result, including preconditions, main flow, branch points, data/state changes, and external IO or async behavior. One-line summaries are review findings. Evidence means project-owned support for a claim, such as source path plus behavior summary, tests, config, migration/schema, API docs, README/wiki, deployment docs, or explicit user confirmation, and it must explain why the source proves the claim. Unknowns means behavior that is unclear, unsupported, conflicting, or owner-dependent; Unknowns are not approved requirements, rules, compatibility promises, or implementation decisions. Feature descriptions, feature flows, business definitions, requirements, and scenarios must use business language and must not contain direct code, pseudocode, call chains, SQL fragments, class/method snippets, or conditional expressions; code identifiers and paths belong only in evidence, source mapping, design baselines, or rule provenance. Multi-language projects must generate or update separate language/runtime standards, such as `docs/rules/java-code-standards.md`, `docs/rules/python-code-standards.md`, or `docs/rules/typescript-react-code-standards.md`. If current code proves stable project-level business terms, lifecycle states, policies, invariants, or cross-capability rules, `/sp-code-to-spec` may generate or update `docs/rules/business-standards.md`. It is not a feature-development workflow: it does not create `openspec/changes/<change-id>/`, write production code, or archive anything. Generated content must be evidence-reviewed and confirmed by the user before file updates.
...
Review authorization rule: `/sp-brainstorm`, `/sp-spec`, `/sp-tasks`, and per-task implementation reviews are owned by the main agent and do not start subagents. Only after `/sp-impl` completes every task and records the main-thread final implementation review should the workflow start two read-only final review subagents when the runtime supports true parallel execution and the current tool has permission. Permission means the current runtime/tool policy allows spawning read-only final review subagents, not that the user must confirm thread startup.

Review performance rule: low-risk, narrow-scope checks may use lightweight review, but must record `review_mode: lightweight`, scope, rationale, artifacts reviewed, skipped full-review areas with reasons, findings, and escalation decision. Final implementation subagent review must be read-only and findings-only; it should not return long explanations or implementation plans. If the current tool cannot run true parallel review and would only run sequential or fake-parallel subagents, record `parallel_unavailable_or_sequential_runtime` and perform the same two final review fallback passes in the main thread.
