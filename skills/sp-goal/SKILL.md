...
4. If `design.md` exists but lacks a recorded user-confirmed E2E required/not-required decision, treat the design/task phase as incomplete. Stop to confirm the E2E decision with the user, then update `design.md`, `tasks.md`, and `tasks-review.md` through the `sp-tasks` workflow before implementation.
5. Starting with the earliest incomplete phase, execute each remaining phase in order:
...
6. After each phase, review the phase artifact before moving forward. Fix in-scope findings before continuing.
7. During implementation, complete tasks one at a time and run both Alignment Review and Security Review after every task.
8. Do not proceed past any unresolved blocking gap, open finding, failed validation, missing standalone verification evidence, missing user-confirmed required real E2E evidence, missing test parameter file, missing coverage evidence, or missing implementation-standard evidence.
9. Stop and report when progress requires user input, new scope, or a spec/design update outside the current approved artifacts.
...
| Tasks | `design.md`, `tasks.md`, and `tasks-review.md` exist, `design.md` records the user-confirmed E2E required/not-required decision, `tasks.md` reflects that E2E decision, and `tasks-review.md` has no unresolved blocking gaps | Run `sp-tasks`; if only E2E confirmation is missing, confirm with the user and update design/tasks/review through `sp-tasks` |
...
- Do not treat an old `design.md` as complete when it lacks user-confirmed E2E required/not-required evidence.
- If `/sp-goal` finds missing E2E confirmation in an existing design, confirm it with the user inside the goal workflow and update design/tasks before implementation.
