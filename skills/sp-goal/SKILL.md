...
11. After all implementation tasks are complete and per-task main-thread findings are closed, require one main-thread final implementation review plus exactly two read-only independent final review agents when true parallel execution is available. Agent 1 reviews requirement/spec/design/code alignment and adversarial evidence. Agent 2 reviews security, implementation standards, regressions, logging, data/API/async/config, test quality, and file-size gates. For `lightweight`, keep the same two final review roles but scope them to the Lightweight Precheck, compact contracts, changed code, tests, verification evidence, security-review applicability, and escalation triggers. If the runtime cannot run true parallel child agents, record the blocker in `review.md` and run the same two scoped final review passes in the main thread. Findings return to the main thread; the main thread confirms issue status, fixes and replies until every required final review has zero unresolved confirmed findings. Confirmed non-blocking, minor, informational, or follow-up final-review findings must also be fixed and re-reviewed before completion.
...
- Do not start independent review threads for `/sp-brainstorm`, `/sp-spec`, `/sp-tasks`, or per-task implementation reviews. These reviews are owned by the main agent.
- Start exactly two read-only independent final review agents only after `/sp-impl` has completed every task and the main-thread final implementation review has been recorded. Do not ask the user for extra authorization when the runtime already allows read-only child-agent review.
...
- If final independent review agents are unavailable, fake-parallel, sequential-only, or the current tool lacks permission, record the blocker in `review.md` and perform the same two scoped final review passes in the main thread.
...
- Only final implementation review requires confirmation across main-thread findings and the two final independent agents or two fallback passes. Confirmed non-blocking/minor/informational/follow-up final-review findings are not deferrable and must be fixed before completion.
...
- Do not complete or archive if final `review.md` lacks one main-thread final implementation review plus two read-only independent final review agents or documented two-pass main-thread fallback. Lightweight lane keeps the same two final review roles, scoped to the lightweight contracts and escalation triggers. Do not complete or archive if any confirmed non-blocking/minor/informational/follow-up final-review finding remains unfixed or lacks re-review closure.
