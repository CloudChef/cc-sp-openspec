...
13. Keep every generated or modified code file at or below 1000 lines. If a target file is already over 1000 lines before the change, record the baseline line count, complete the approved functionality first, verify it, then refactor/split the oversized file before marking the task complete and re-run affected verification.
...
34. For `full`, when true parallel execution is available, start Independent Review Thread 1 and Independent Review Thread 2 in one parallel dispatch. If the tool cannot dispatch both concurrently, record the blocker and run the same two scoped review passes in the main thread. For `lightweight`, use one reusable read-only reviewer when true parallel execution is available; if not, record fallback and run the same scoped review in the main thread.
35. For `full`, Independent Review Thread 1 reviews requirement/spec/design/code alignment across all requirements, specs, design, tasks, code, tests, counterexample matrices, masked-test analysis, broad-qualifier audit, Decision Chain Trace, Evidence Capture Timing Audit, Deterministic Sort Audit, and verification evidence; Independent Review Thread 2 reviews security, implementation standards, regression, logging/traceability, data handling, API IO/async, test quality, file size, and configuration. For `lightweight`, the reusable read-only reviewer checks the Lightweight Precheck, compact contracts, changed code, tests, verification evidence, security-review applicability, and escalation triggers. Reviewers must return concise findings only to the main thread and must not edit files.
36. Cross-validate main-thread final review findings and all required independent-review or fallback findings. Confirm whether each finding is valid, duplicate, false positive, already fixed, requires full-lane escalation, or requires customer/user decision. Record the confirmation decision and evidence in `review.md`.
37. The main thread must discuss, triage, fix, and reply to every confirmed finding from the main-thread final review and required independent review threads or fallback passes. After fixes, re-run affected tests/verification and re-run the relevant review or fallback pass until all required reviews report zero unresolved findings.
38. For `full`, Final Code Review Pass 1 is satisfied by Independent Review Thread 1 when true parallel execution is available, or by the same scoped main-thread fallback pass when it is not. It must review the complete code change against specs, design, design review, tasks, rules, architecture, tests, requirement-to-test mapping, counterexample matrices, masked-test analyses, broad-qualifier audits, Decision Chain Trace, Evidence Capture Timing Audit, Deterministic Sort Audit, coverage, standalone verification, real E2E evidence, browser/UI QA evidence, comment/logging/traceability evidence, encoding/no-mojibake evidence, reuse/common logic, fallback/compatibility constraints, parameter/data-object constraints, and changed file sizes.
39. For `full`, Final Code Review Pass 2 is satisfied by Independent Review Thread 2 when true parallel execution is available, or by the same scoped main-thread fallback pass when it is not. It must review security, data handling, authorization, logging/`trace_id`/sensitive-data exposure, encoding/no-mojibake regressions, API IO, async behavior, configuration, dependencies, test quality, masked-test risks, decision-chain masking regressions, evidence-capture timing regressions, deterministic-sort regressions, broad-qualifier regressions, narrower code qualifiers, and remaining rule violations. For `lightweight`, the single scoped final reviewer satisfies the independent final review requirement unless it finds an escalation trigger.
40. Create or update final `review.md` only after the main-thread final review, required independent review threads or documented fallback passes, cross-validation, and all required reviews have zero unresolved findings. For `full`, final review includes Requirement Counterexample Matrix, Masked-Test Analysis, Broad-Qualifier Audit, Decision Chain Trace, Evidence Capture Timing Audit, Deterministic Sort Audit, independent thread findings, cross-validation decisions, and main-thread responses. For `lightweight`, final review includes Lightweight Precheck, compact-contract alignment, verification evidence, security-review applicability, escalation-trigger check, reviewer findings or fallback evidence, cross-validation decisions, and main-thread responses.
...
- If an existing target file is already over 1000 lines before the change, do not start with broad refactoring. Record the baseline line count, finish the approved functional change first, verify it, then refactor/split the oversized file and re-run affected tests, standalone verification, and relevant E2E/UI checks before marking the task complete.
...
- Do not finalize implementation until the final review required by the workflow lane is complete. Full lane requires the main-thread full implementation review plus two independent final review threads or documented main-thread fallback passes. Lightweight lane requires one scoped main-thread final review plus one reusable read-only reviewer or documented main-thread fallback, unless escalation triggers require switching to full. In both lanes, cross-validation must confirm every finding and all confirmed findings must have zero unresolved status after main-thread fixes and re-review.
...
For full lane, include every section below. For lightweight lane, keep the same review structure where useful, but record untriggered full-only matrix/audit sections as not applicable with evidence, and replace full independent review sections with the scoped main final review plus reusable reviewer or documented fallback evidence.
...
- Final completion requires final review evidence according to the workflow lane. Full lane requires one main-thread full implementation review plus two independent final review threads on the complete implementation diff when true parallel execution is available, or the same two scoped final fallback passes in the main thread. Lightweight lane requires one scoped main-thread final review plus one reusable read-only reviewer when true parallel execution is available, or the same scoped fallback review in the main thread. `review.md` must record cross-validation confirming every finding and all confirmed findings fixed by the main thread and re-reviewed to zero open findings.
...
Use Superpower review skills when available. Request each required per-task review, main-thread final implementation review, and independent final review thread with `superpowers:requesting-code-review`; when findings are returned, process, verify, and fix them with `superpowers:receiving-code-review` before re-review. For lightweight lane, request scoped lightweight review unless escalation triggers require full review.
...
- Findings from independent threads return to the main thread. The main thread cross-validates its own findings against both independent-thread findings, confirms issue status, fixes and replies to confirmed findings, then re-runs the relevant review until no unresolved findings remain.

Independent review threads are read-only and findings-only. They must return concise structured findings with priority, evidence location, impact, and suggested fix. They must not return long summaries, restated context, implementation plans, or broad explanations.

Reuse review threads when the runtime supports persistent reusable threads. For the same `change-id`, repository/worktree, and review role, keep using the existing read-only reviewer instead of opening a new thread for every implementation task. Typical reusable roles are Alignment Review, Security Review, Final Alignment Review, and Final Security Review.

Every reused reviewer invocation must receive the current task or final-review scope, fresh artifacts/diff, checklist, and evidence references. The reviewer must return findings for the current scope only and must not rely on unstated prior context. Record the reviewer role, thread identifier/name when available, reused/new status, reviewed scope, artifacts provided, findings, and restart/fallback reason in `task-reviews.md` or `review.md`.
...
- Main-thread findings not reported by independent threads
- Independent-thread findings not reported by the main thread
- Duplicates between independent threads
- Conflicts between the two independent threads
...
Start required independent final review threads automatically when the runtime supports true parallel execution and the current tool has permission. Permission means the current runtime/tool policy allows spawning read-only review threads, not that the user must confirm thread startup. Do not launch sequential or fake-parallel subagents just to satisfy the independent review requirement.

If independent review threads are unavailable, fake-parallel, sequential-only, or the current tool lacks permission to start them, record `parallel_unavailable_or_sequential_runtime` or the applicable blocker and perform the lane-required fallback in the main thread. Full lane requires the same two scoped final fallback passes. Lightweight lane requires the same single scoped fallback review unless escalation triggers require full review. Do not stop to request extra user approval, and do not skip any lane-required review pass.
...
- Changed/generated code files are at or below 1000 lines, or an existing oversized target file has documented baseline evidence plus completed post-functionality refactor/split evidence showing the final touched files meet the limit.
...
Resolve all findings from required review passes before starting the next task.

After all tasks are complete, run final implementation review according to the lane. Full lane requires one main-thread full implementation review and two additional independent final review threads on the complete implementation diff when true parallel execution is available; start both independent threads in one true parallel dispatch, or record fallback and run the same two scoped passes in the main thread. Lightweight lane requires one scoped main-thread final review and, when true parallel execution is available, one reusable read-only reviewer for the same compact scope; otherwise record fallback and run the same scoped review in the main thread. Record the main review, independent threads or fallback passes, cross-validation/confirmation decisions, all findings, all main-thread fixes/replies, and re-review closure in `review.md`; implementation is not complete until all required final reviews have zero open confirmed findings after required fixes and re-review.
